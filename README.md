...
Paste book URL(s) — comma-separated for batch — and the tool shows metadata for each right away:
...
Then it asks if you'd like to download each book. If you say yes:

1. **Session ID** — grabbed once at the start from browser cookies via `rookiepy` if available. Falls back to manual paste or `SESSION_ID` env variable.
